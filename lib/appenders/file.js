...
function fileAppender (file, layout, logSize, numBackups) {
    ...
    function flushBuffer(sync) {
        while (logEventBuffer.length > 0 && (canWrite || sync)) {
            writeToLog(logEventBuffer.shift(), sync);
        }
    }
    ...
    function writeToLog(loggingEvent, sync) {
        if (!logFile.writable) return;

        var logMessage = layout(loggingEvent)+'\n';
        //not entirely accurate, but it'll do.
        bytesWritten += logMessage.length;
        if (sync) {
          fs.writeSync(logFile.fd,logMessage, "utf8");
        } else {
