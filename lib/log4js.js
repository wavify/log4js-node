...
function getLogger (loggerCategoryName) {
  ...
  if (!hasLogger(loggerCategoryName)) {
    ...
    var index = loggerCategoryName.lastIndexOf('.');
    if (index <= 0) {
      // Create the logger for this name if it doesn't already exist
      loggers[loggerCategoryName] = new Logger(loggerCategoryName, level, null);
    } else {
      var parentLoggerCategoryName = loggerCategoryName.slice(0, index);
      loggers[loggerCategoryName] = new Logger(loggerCategoryName, level, getLogger(parentLoggerCategoryName));
    }
