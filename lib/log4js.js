...
//suppress EventEmitter memory leak warning for exit event listeners
...
function configureLevels(levels) {
    ...
    if (levels) {
        var sortedCategory = Object.keys(levels).sort();
        for (var i in sortedCategory) {
            var category = sortedCategory[i];
            var level = levels[category];
            getLogger(category).setLevel(level);
            for (l in loggers) {
                if (loggers.hasOwnProperty(l) && l.indexOf(category) == 0) {
                    loggers[l].setLevel(level);
                }
            }
        }
    }
